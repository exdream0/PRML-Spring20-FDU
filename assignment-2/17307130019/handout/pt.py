...
class myPTRNNModel(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.RNN(64, 64, 2)    # 输入维度64，隐藏层神经元数64，网络层数2
...
class myAdvPTRNNModel(nn.Module):
    ...
    def __init__(self, n_layer = 2, model = "LSTM"):
        ...
        if model == "LSTM":
            self.rnn = nn.LSTM(64, 64, n_layer)
        if model=="GRU":
            self.rnn = nn.GRU(64, 64, n_layer)
        else:
            self.rnn = nn.RNN(64, 64, n_layer)
...
def train(steps, model, optimizer, max_len = 10, test_len = 10):
    loss = 0.0
    accuracy = 0.0
    end_num = 5 * 10 ** (max_len - 1)
...
def evaluate(model, max_len = 10):
    start_num = 5 * 10 ** (max_len - 1) + 1
    end_num = 10 ** (max_len) - 1

    datas = gen_data_batch(batch_size=2000, start=start_num, end=end_num)
    Nums1, Nums2, results = prepare_batch(*datas, maxlen=max_len + 1)
    with torch.no_grad():
...
def pt_main(n_train = 3000, max_len = 10):
    model = myPTRNNModel()
    # 构建优化器对象，lr为步长因子
...
def pt_adv_main(n_train = 3000, max_len = 10, test_len = 10, n_layer = 3, typ = "LSTM", my_lr = 0.01):
    '''
    Please finish your code here.
    '''
    ...
    train(n_train, model, optimizer, max_len, test_len)
    # evaluate(model)
    draw(model)

if __name__ == '__main__':
    pt_main(500, 100)
    pt_adv_main(500, 100, 3, "LSTM", 0.01)